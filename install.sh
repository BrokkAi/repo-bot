...
fail() {
    printf 'brokk-repo-bot installer: %s\n' "$*" >&2
    exit 1
}
...
main() {
    ...
    releases=https://github.com/BrokkAi/repo-bot/releases
    version=${1:-latest}
    ...
    [ ! -d "$install_dir/brp" ] || fail 'installation target is a directory'
    temporary=$(mktemp -d)
    staged=
    trap 'rm -rf "$temporary"; if [ -n "$staged" ]; then rm -f "$staged"; fi' 0
    trap 'exit 1' HUP INT TERM

    asset=brokk-repo-bot-$version-$os-$arch.tar.gz
    printf 'Downloading brokk-repo-bot %s for %s/%s...\n' "$version" "$os" "$arch"
    ...
    mkdir -p "$install_dir"
    staged=$(mktemp "$install_dir/.brokk-repo-bot.XXXXXX")
    # Extract only the binary to a fresh file; never follow archive paths or links.
    tar -xzOf "$temporary/$asset" brp > "$staged" || fail 'could not extract brp'
    [ -s "$staged" ] || fail 'archive contains an empty binary'
    chmod 755 "$staged"
    mv -f "$staged" "$install_dir/brp"
    staged=
    printf 'Installed brokk-repo-bot %s to %s/brp\n' "$version" "$install_dir"
    case ":${PATH:-}:" in
        *":$install_dir:"*) ;;
        *) printf 'Add %s to your PATH, then run brp --help.\n' "$install_dir" ;;
    esac
}
